...
session.commit()
pizzaRecipe = Recipe(name="yummy pizza pie", description="it's molto bene!", difficulty="Easy", cuisine_id=pizza.cuisine_id, user_id=testUser.id)
...
session.commit()
ingredient1 = Ingredient(name="salsa", recipe_id=pizzaRecipe.id, amount=1, user_id=testUser.id,unit="tsp")
